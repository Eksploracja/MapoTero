...
begin
  ...
  try
    ...
    if (MaxPixelsForBiggestSide=0) or
      ...
      // no resize necessary
      Pic.SaveAs(Stream,Format,CompressionQuality) else begin
  ...
  EMR_EXTTEXTOUTW: begin
    with PEMRExtTextOut(Rec)^ do begin
      ...
      if emrtext.offDx=0 then
        ...
        SetPositions(RF.X,RF.Y,F32,pointer(cardinal(Rec)+emrtext.offDx),emrText.nChars);
      ...
      Ref.gdip.DrawDriverString(Ref.graphics,PWideChar(cardinal(Rec)+emrtext.offString),
        emrtext.nChars, font, Ref.GetCachedSolidBrush(fontColor), F32, flags, 0);
      if fontspec.angle<>0 then begin
        Ref.gdip.SetWorldTransform(Ref.graphics,matrixOrg); // restore previous
        Ref.gdip.DeleteMatrix(matrixOrg);
      end else
      // DrawDriverString doesn't handle those -> GDI+ does not work :(
